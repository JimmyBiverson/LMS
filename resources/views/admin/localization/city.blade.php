...
<div class="bg-white rounded-xl shadow-sm mb-6">
    <div class="p-6 border-b border-gray-100">
        <h3 class="font-bold text-heading">Add New City</h3>
    </div>
    <div class="p-6">
        <form method="POST" action="/admin/localization/city" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            @csrf
            ...
            <select name="state_id" required class="px-4 py-2.5 border border-gray-200 rounded-lg text-sm">
            ...
            <button type="submit" class="px-6 py-2.5 bg-primary text-white font-semibold rounded-lg hover:opacity-90 text-sm md:col-span-2 lg:col-span-3">Add City</button>
        </form>
    </div>
</div>

<div class="bg-white rounded-xl shadow-sm">
    <div class="p-6 border-b border-gray-100"><h3 class="font-bold text-heading">Cities</h3></div>
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            <thead><tr class="bg-gray-50 text-heading/60 text-xs uppercase tracking-wider">
                <th class="text-left py-4 px-6 font-semibold">#</th>
                <th class="text-left py-4 px-6 font-semibold">Name</th>
                <th class="text-left py-4 px-6 font-semibold">State</th>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="py-4 px-6 text-heading/70">{{ $c->state->name ?? '--' }} ({{ $c->state->country->name ?? '' }})</td>
                    ...
                    <td class="py-4 px-6">
                        <div class="flex items-center gap-2">
                            ...
                            <form method="POST" action="/admin/localization/city/{{ $c->id }}/delete" x-data @submit.prevent="if(confirm('Delete this city?')) $el.submit()">
                                @csrf
                ...
                @empty
                <tr><td colspan="5" class="py-8 text-center text-heading/50 text-sm">No cities found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
<div x-data="{ open: false, item: {} }"
    ...
    <div class="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        <div class="p-6 border-b border-gray-100 flex items-center justify-between">
            <h3 class="font-bold text-heading">Edit City</h3>
            <button @click="open = false" class="text-heading/40 hover:text-heading"><i class="ri-close-line text-xl"></i></button>
        </div>
        <form method="POST" :action="'/admin/localization/city/' + item.id + '/update'" class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
            @csrf
            ...
            <select name="state_id" required x-model="item.state_id" class="px-4 py-2.5 border border-gray-200 rounded-lg text-sm">
                <option value="">Select State</option>
                @foreach($states as $state)
                <option value="{{ $state->id }}">{{ $state->name }} ({{ $state->country->name ?? '' }})</option>
                @endforeach
