...
<div class="max-w-4xl">
    <div class="bg-white rounded-xl shadow-sm p-6">
        ...
        @endif
        <!-- FIXED: now POSTs to the configured instructor course route -->
        <form method="POST" action="{{ url('instructor/courses') }}" enctype="multipart/form-data" class="space-y-5">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div><label class="block text-sm font-semibold text-heading mb-1">Category *</label><select name="category_id" class="w-full px-4 py-3 rounded-lg border border-heading/10 text-sm focus:outline-none focus:border-primary"><option value="">Select category</option>@foreach($categories ?? [] as $cat)<option value="{{ $cat->id }}" @selected(old('category_id') == $cat->id)>{{ $cat->name }}</option>@endforeach</select></div>
                <div><label class="block text-sm font-semibold text-heading mb-1">Level</label><select name="level_id" class="w-full px-4 py-3 rounded-lg border border-heading/10 text-sm focus:outline-none focus:border-primary"><option value="">Select level</option>@foreach($levels ?? [] as $level)<option value="{{ $level->id }}" @selected(old('level_id') == $level->id)>{{ $level->name }}</option>@endforeach</select></div>
            </div>
            <div><label class="block text-sm font-semibold text-heading mb-1">Tags</label>
                <div class="flex flex-wrap gap-3 mt-1">@foreach($tags ?? [] as $tag)<label class="flex items-center gap-2 cursor-pointer select-none"><input type="checkbox" name="tags[]" value="{{ $tag->id }}" {{ in_array($tag->id, old('tags', [])) ? 'checked' : '' }} class="w-4 h-4 text-primary focus:ring-primary rounded"><span class="text-sm text-heading/80">{{ $tag->name }}</span></label>@endforeach</div>
            </div>
            ...
            <div>
                ...
                <div class="flex gap-6 mb-4">
                    <label class="flex items-center gap-2 cursor-pointer select-none">
                        <input type="radio" name="payment_type" value="free" onclick="togglePaymentType('free')" {{ old('payment_type', 'free') === 'free' ? 'checked' : '' }} class="w-4 h-4 text-primary focus:ring-primary">
                        <span class="text-sm font-semibold text-heading">Free</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer select-none">
                        <input type="radio" name="payment_type" value="paid" onclick="togglePaymentType('paid')" {{ old('payment_type') === 'paid' ? 'checked' : '' }} class="w-4 h-4 text-primary focus:ring-primary">
                        <span class="text-sm font-semibold text-heading">Paid</span>
                    </label>
                </div>
                <div id="priceFields" class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-4" style="{{ old('payment_type') === 'paid' ? '' : 'display:none' }}">
                    <div>
            ...
            <div class="relative">
                ...
                <div id="thumbnailPreview" class="mt-3 hidden">
                    <img id="previewImage" src="" alt="Preview" loading="lazy" class="max-w-xs max-h-40 rounded-lg mx-auto" />
            ...
            <div><label class="block text-sm font-semibold text-heading mb-1">Preview Video</label>
            <div class="relative">
                <div id="previewVideoDropZone" class="border-2 border-dashed border-heading/20 rounded-lg p-6 text-center cursor-pointer hover:border-primary hover:bg-primary/5 transition-all" style="min-height: 120px; display: flex; align-items: center; justify-content: center;">
                    <div class="text-center">
                        <i class="ri-video-add-line text-3xl text-primary mb-2 block"></i>
                        <p class="text-sm font-semibold text-heading mb-1">Drag video here or click to upload</p>
                        <p class="text-xs text-heading/50">MP4, MOV, AVI, WebM or OGG • Max 100MB</p>
                    </div>
                    <input id="previewVideoInput" name="preview_video" type="file" accept="video/mp4,video/quicktime,video/x-msvideo,video/webm,video/ogg" class="absolute inset-0 opacity-0 cursor-pointer" />
                </div>
                <div id="previewVideoName" class="mt-2 hidden">
                    <p class="text-xs text-heading/60 flex items-center gap-1"><i class="ri-video-line text-primary"></i> <span id="previewVideoLabel"></span></p>
                    <button type="button" id="removePreviewVideo" class="mt-1 px-3 py-1 bg-red-100 text-red-700 text-xs font-semibold rounded hover:bg-red-200 transition-all">Remove</button>
                </div>
                @error('preview_video')
                    <div class="mt-2 p-3 bg-red-50 border border-red-200 text-red-700 text-sm rounded">{{ $message }}</div>
                @enderror
...
function togglePaymentType(type) {
    var fields = document.getElementById('priceFields');
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (checked) togglePaymentType(checked.value);
    ...
    const pvDropZone = document.getElementById('previewVideoDropZone');
    const pvInput = document.getElementById('previewVideoInput');
    const pvName = document.getElementById('previewVideoName');
    const pvLabel = document.getElementById('previewVideoLabel');
    const pvRemove = document.getElementById('removePreviewVideo');

    if (pvDropZone) {
        ['dragenter', 'dragover', 'dragleave', 'drop'].forEach(evt => {
            pvDropZone.addEventListener(evt, function(e) { e.preventDefault(); e.stopPropagation(); }, false);
        });
        ['dragenter', 'dragover'].forEach(evt => {
            pvDropZone.addEventListener(evt, function() { pvDropZone.classList.add('border-primary', 'bg-primary/10'); }, false);
        });
        ['dragleave', 'drop'].forEach(evt => {
            pvDropZone.addEventListener(evt, function() { pvDropZone.classList.remove('border-primary', 'bg-primary/10'); }, false);
        });
        pvDropZone.addEventListener('drop', function(e) {
            pvInput.files = e.dataTransfer.files;
            handlePreviewVideoSelect({ target: { files: e.dataTransfer.files } });
        }, false);
        pvDropZone.addEventListener('click', () => pvInput.click());
    }

    pvInput?.addEventListener('change', handlePreviewVideoSelect);

    function handlePreviewVideoSelect(e) {
        const file = e.target.files?.[0];
        if (!file) return;
        const valid = ['video/mp4', 'video/quicktime', 'video/x-msvideo', 'video/webm', 'video/ogg'];
        if (!valid.includes(file.type)) {
            alert('Please upload a valid video file (MP4, MOV, AVI, WebM, or OGG)');
            pvInput.value = '';
            return;
        }
        ...
        pvLabel.textContent = file.name;
        pvName.classList.remove('hidden');
    }

    pvRemove?.addEventListener('click', function() {
        pvInput.value = '';
