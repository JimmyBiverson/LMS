        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    @php 
                        $resL = $e->course?->getResumeLesson(); 
                    @endphp
                    <td class="py-4 px-6 text-right">
                        @if($resL)
                            <a href="/courses/{{ $e->course?->slug }}/lessons/{{ $resL->id }}" class="text-primary text-sm font-semibold hover:underline inline-flex items-center gap-1 group">
                                Resume <i class="ri-arrow-right-line group-hover:translate-x-1 transition-transform"></i>
                            </a>
                        @else
                            <a href="/courses/{{ $e->course?->slug ?? $e->course_id }}" class="text-primary text-sm font-semibold hover:underline">View</a>
                        @endif
                    </td>
                </tr>
                @empty
