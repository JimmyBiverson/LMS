...
@section('title', 'Course Note')
@section('page-title', 'Course Note')
...
<div class="bg-white rounded-xl shadow-sm p-6 space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <a href="{{ route('dashboard.course-notes.index') }}" class="px-4 py-2 border border-gray-200 rounded-full">Back</a>
    </div>
    ...
    @if($courseNote->attachment_path && $courseNote->allow_download)
        <div class="rounded-lg border border-gray-100 p-4">
            <p class="font-semibold text-heading">Attachment</p>
            <a href="{{ route('dashboard.course-notes.download', $courseNote) }}" class="text-primary hover:underline">Download {{ $courseNote->download_label }}</a>
        </div>
    @endif
