...
@section('user-name', match(auth()->user()->role) { 'admin' => 'Admin', 'staff' => 'Staff', 'instructor' => 'Instructor', 'organization' => 'Organization', default => 'Student' })
@section('sidebar')
@php
...
$notifPrefix = match(auth()->user()->role) {
    'admin', 'staff' => 'admin',
...
<div class="bg-white rounded-xl shadow-sm">
    <div class="p-6 border-b border-gray-100 flex items-center justify-between">
        ...
        @if($notifications->where('is_read', false)->count())
            <form method="POST" action="{{ url("$notifPrefix/notifications/read-all") }}">
            @csrf
    ...
    <div class="divide-y divide-gray-100">
        ...
        <div class="p-5 flex items-start gap-4 {{ !$n->is_read ? 'bg-primary/5' : 'group' }}">
            @if($n->link)
            <a href="{{ $n->link }}" class="flex items-start gap-4 flex-1 min-w-0 no-underline text-inherit">
            @endif
            ...
            @endif
            @if(!$n->is_read)
            <form method="POST" action="{{ url("$notifPrefix/notifications/$n->id/read") }}">
                @csrf
