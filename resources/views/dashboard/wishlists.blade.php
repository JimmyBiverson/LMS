...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    ...
    <div class="bg-white rounded-xl shadow-sm overflow-hidden group hover:shadow-lg transition-all duration-300">
        <div class="relative">
            @if($w->course?->thumbnail_url)
                <img src="{{ $w->course->thumbnail_url }}" alt="{{ $w->course->title }}" loading="lazy" class="w-full h-48 object-cover">
            @else
            ...
            <div class="flex items-center justify-between mt-4 pt-4 border-t border-gray-100">
                <span class="font-bold text-heading">
                    ...
                    @elseif($w->course?->sale_price)
                        <span class="text-heading/40 line-through text-xs mr-1">{{ \App\Helpers\CurrencyHelper::format((float)($w->course?->price ?? 0)) }}</span>
                        {{ \App\Helpers\CurrencyHelper::format((float)($w->course?->sale_price ?? 0)) }}
                    @else
                        {{ \App\Helpers\CurrencyHelper::format((float)($w->course?->price ?? 0)) }}
                    @endif
                </span>
