...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
    ...
    <div class="bg-gradient-to-br from-amber-600 to-red-500 rounded-xl p-5 shadow-lg text-white">
        <div class="flex items-center justify-between"><div><p class="text-amber-100 text-xs font-semibold uppercase tracking-wider">Est. Revenue</p><p class="text-3xl font-extrabold mt-1">${{ number_format($courses->sum(fn($c)=>$c->enrollments_count * ($c->sale_price ?? $c->price)), 0) }}</p></div><div class="w-12 h-12 rounded-xl bg-white/20 flex items-center justify-center text-white"><i class="ri-money-dollar-circle-line text-xl"></i></div></div>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-amber-50/30 transition-colors">
                        ...
                        <td class="py-3 px-4 text-heading/70">{{ $c->price > 0 ? '$'.number_format((float)$c->price,2) : 'Free' }}</td>
