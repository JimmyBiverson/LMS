...
<div class="bg-white rounded-xl shadow-sm mb-6">
    <div class="p-6 border-b border-gray-100">
        <h3 class="font-bold text-heading">Add New Email Template</h3>
    </div>
    <div class="p-6">
        <form method="POST" action="/admin/email-template" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            @csrf
            ...
            <input type="text" name="subject" placeholder="Subject" required class="px-4 py-2.5 border border-gray-200 rounded-lg text-sm">
            <select name="status" class="px-4 py-2.5 border border-gray-200 rounded-lg text-sm">
                <option value="active">Active</option>
                <option value="inactive">Inactive</option>
            </select>
            <textarea name="body" placeholder="Body" rows="4" class="px-4 py-2.5 border border-gray-200 rounded-lg text-sm md:col-span-2 lg:col-span-3"></textarea>
            <button type="submit" class="px-6 py-2.5 bg-primary text-white font-semibold rounded-lg hover:opacity-90 text-sm md:col-span-2 lg:col-span-3">Add Template</button>
        </form>
    </div>
</div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="py-4 px-6">
                        <div class="flex items-center gap-2">
                            ...
                            <form method="POST" action="/admin/email-template/{{ $t->id }}/delete" x-data @submit.prevent="if(confirm('Delete this template?')) $el.submit()">
                                @csrf
                ...
                @empty
                <tr><td colspan="5" class="py-8 text-center text-heading/50 text-sm">No email templates found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
<div x-data="{ open: false, item: {} }"
    ...
    <div class="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        <div class="p-6 border-b border-gray-100 flex items-center justify-between">
            <h3 class="font-bold text-heading">Edit Email Template</h3>
            <button @click="open = false" class="text-heading/40 hover:text-heading"><i class="ri-close-line text-xl"></i></button>
        </div>
        <form method="POST" :action="'/admin/email-template/' + item.id + '/update'" class="p-6 grid grid-cols-1 gap-4">
            @csrf
            ...
            <input type="text" name="subject" placeholder="Subject" required x-model="item.subject" class="px-4 py-2.5 border border-gray-200 rounded-lg text-sm">
            <textarea name="body" placeholder="Body" rows="5" x-model="item.body" class="px-4 py-2.5 border border-gray-200 rounded-lg text-sm"></textarea>
