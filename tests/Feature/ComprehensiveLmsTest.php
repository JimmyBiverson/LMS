...
class ComprehensiveLmsTest extends TestCase
{
    ...
    public function test_become_instructor_standalone_works()
    {
        ...
        $response->assertRedirect('/instructor/pending-approval');
        $this->assertAuthenticated();
    ...
    public function test_course_list_and_search_and_bundle_pages_render_without_errors()
    {
        $instructor = $this->createUser('instructor');
        $course = $this->createCourse([
            'user_id' => $instructor->id,
            ...
            'status' => 'Active',
            'preview_video' => 'courses/preview-videos/test.mp4',
        ]);
        
        // Add a preview lesson
        $lesson = Lesson::create([
            'course_id' => $course->id,
            ...
            'status' => 'published',
            'is_free_preview' => true,
            'video_file' => 'lessons/videos/test.mp4',
        ]);

        // 1. Check Courses index page
        $response = $this->get('/courses');
        $response->assertStatus(200);
        $response->assertSee('Advanced PHP Programming');

        // 2. Check Search page
        $response = $this->get('/search?q=Advanced');
        $response->assertStatus(200);
        $response->assertSee('Advanced PHP Programming');

        // 3. Check Bundle details page
        ...
        $response = $this->get('/bundles/web-dev-bundle');
        $response->assertStatus(200);
        $response->assertSee('Advanced PHP Programming');
    }
}
