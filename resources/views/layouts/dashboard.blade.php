    ...
    <div class="flex-1 flex flex-col min-w-0">
        <header class="h-16 {{ $roleHeaderBg ?? 'bg-white' }} border-b {{ $roleSidebarBorder }} flex items-center justify-between px-4 lg:px-6 sticky top-0 z-10">
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="flex items-center gap-2 cursor-pointer relative" x-data="{ open: false }" @click.outside="open = false">
                    <button type="button" @click="open = !open" class="flex items-center gap-2 cursor-pointer">
                        ...
                        <i class="ri-arrow-down-s-line text-heading/60 text-sm transition-transform duration-300" :class="{ 'rotate-180': open }"></i>
                    </button>
                    <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="absolute top-full right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-gray-100 py-2 z-50">
                        @php
