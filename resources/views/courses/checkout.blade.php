                ...
                <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden mb-6">
                    <div class="flex p-4 gap-4">
                        @if($course->thumbnail_url)
                            <img src="{{ $course->thumbnail_url }}" alt="{{ $course->title }}" width="96" height="80" loading="lazy" class="w-24 h-20 object-cover rounded-lg shrink-0">
                        @else
                    ...
                    <div class="space-y-2 text-sm">
                        <div class="flex justify-between">
                            ...
                            @elseif($course->sale_price)
                                <span class="text-heading/40 line-through">{{ \App\Helpers\CurrencyHelper::format((float)$course->price) }}</span>
                            @else
                                <span class="font-semibold">{{ \App\Helpers\CurrencyHelper::format((float)$course->price) }}</span>
                            @endif
                        </div>
                        @if($course->sale_price)
                        <div class="flex justify-between">
                            <span class="text-heading/60">Sale Price</span>
                            <span class="text-red-500 font-semibold">-{{ \App\Helpers\CurrencyHelper::format((float)$course->price - (float)$course->sale_price) }}</span>
                        </div>
                        @endif
                        <div class="border-t border-gray-100 pt-2 flex justify-between font-bold text-heading">
                            ...
                            @else
                                <span>{{ \App\Helpers\CurrencyHelper::format((float)($course->sale_price ?? $course->price)) }}</span>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <form method="POST" action="/enroll/{{ $course->id }}">
                    ...
                    <div class="flex items-center gap-4">
                        <button type="submit"
                            ...
                            @else
                                Confirm & Enroll - {{ \App\Helpers\CurrencyHelper::format((float)($course->sale_price ?? $course->price)) }}
                            @endif
                        </button>
