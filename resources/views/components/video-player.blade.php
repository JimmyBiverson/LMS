...
@props(['lesson', 'autoplay' => true, 'previewSeconds' => 8])
    ...
    @elseif($hasVideo)
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                if (video) {
                    const previewSeconds = parseInt(video.dataset.previewSeconds);
            ...
            function showPreviewLimitModal() {
                // Simple preview limit notification
                console.log('Preview limit reached. User needs to be enrolled.');
            }
        </script>
        @endif
    @else
...
<div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    <button onclick="downloadVideo(this)" class="flex items-center gap-3 p-4 rounded-xl border border-purple-200 bg-purple-50 hover:bg-purple-100 transition-colors group cursor-pointer">
        ...
        <div class="flex-1">
            ...
            <p class="text-xs text-heading/60">Coming soon</p>
        </div>
        <i class="ri-download-line text-heading/40 group-hover:text-primary transition-colors"></i>
    </button>
    @endif
...
function downloadVideo(btn) {
    ...
    alert('Video download feature coming soon!');
}
</script>
