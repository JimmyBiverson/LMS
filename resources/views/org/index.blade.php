...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
    ...
    <div class="bg-gradient-to-br from-indigo-400 to-indigo-600 rounded-xl p-5 shadow-lg text-white">
        <div class="flex items-center justify-between"><div><p class="text-indigo-100 text-xs font-semibold uppercase tracking-wider">Instructors</p><p class="text-3xl font-extrabold mt-1">{{ \App\Models\User::where('role', 'instructor')->count() }}</p></div><div class="w-12 h-12 rounded-xl bg-white/20 flex items-center justify-center text-white"><i class="ri-team-line text-xl"></i></div></div>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center justify-between p-3 rounded-lg bg-fuchsia-50"><span class="text-sm text-fuchsia-700 font-semibold">Total Revenue</span><span class="text-sm font-bold text-fuchsia-900">${{ number_format($courses->sum(fn($c)=>$c->enrollments_count * ($c->sale_price ?? $c->price)), 0) }}</span></div>
            </div>
        </div>
    </div>
</div>
