    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                @for($i=1;$i<=4;$i++)
                <tr class="hover:bg-gray-50"><td class="py-4 px-6 text-heading/70">{{ $i }}</td><td class="py-4 px-6 font-semibold text-heading uppercase">SAVE{{ $i }}0</td><td class="py-4 px-6 text-heading/70">{{ 10+$i*5 }}%</td><td class="py-4 px-6 text-heading/70">Percentage</td><td class="py-4 px-6 text-heading/70">{{ 10+$i*5 }}</td><td class="py-4 px-6"><span class="px-3 py-1 rounded-full text-xs font-bold bg-green-100 text-green-700">Active</span></td></tr>
                @endfor
            </tbody>
        </table>
    </div>
