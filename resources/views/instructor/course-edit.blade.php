            ...
            <div class="relative">
                ...
                <div id="thumbnailPreview" class="mt-3 {{ $course->thumbnail && $course->thumbnail !== 'N/A' ? '' : 'hidden' }}">
                    @if($course->thumbnail && $course->thumbnail !== 'N/A')
                        <img id="previewImage" src="{{ $course->thumbnail_url }}" alt="Current Thumbnail" loading="lazy" class="max-w-xs max-h-40 rounded-lg mx-auto" />
                    @else
            ...
            <div class="relative">
                <div id="previewVideoDropZone" class="border-2 border-dashed border-heading/20 rounded-lg p-6 text-center cursor-pointer hover:border-primary hover:bg-primary/5 transition-all" style="min-height: 120px; display: flex; align-items: center; justify-content: center;">
                    <div class="text-center">
                        ...
                        <p class="text-xs text-green-600 mt-2 font-semibold"><i class="ri-checkbox-circle-fill"></i> Current video: {{ basename($course->preview_video) }}</p>
                        @endif
                    </div>
                ...
                <div id="previewVideoName" class="mt-2 {{ $course->preview_video ? '' : 'hidden' }}">
                    <p class="text-xs text-heading/60 flex items-center gap-1"><i class="ri-video-line text-primary"></i> <span id="previewVideoLabel">{{ $course->preview_video ? basename($course->preview_video) : '' }}</span></p>
                    <input type="hidden" name="remove_preview_video" id="removePreviewVideoInput" value="0">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    pvRemove?.addEventListener('click', function() {
        ...
        document.getElementById('removePreviewVideoInput').value = '1';
        pvRemoved = true;
    });
});
</script>
