...
<div class="bg-white rounded-xl shadow-sm mb-6">
    <div class="p-6 border-b border-gray-100">
        <h3 class="font-bold text-heading">Add New Language</h3>
    </div>
    <div class="p-6">
        <form method="POST" action="/admin/site-language" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            @csrf
            ...
            <button type="submit" class="px-6 py-2.5 bg-primary text-white font-semibold rounded-lg hover:opacity-90 text-sm md:col-span-2 lg:col-span-3">Add Language</button>
        </form>
    </div>
</div>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="py-4 px-6 text-heading/70">{{ $l->code }}</td>
                    <td class="py-4 px-6 text-heading/70">{{ $l->is_default ? 'Yes' : 'No' }}</td>
                    <td class="py-4 px-6"><span class="px-3 py-1 rounded-full text-xs font-bold {{ $l->status=='active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">{{ ucfirst($l->status) }}</span></td>
                    <td class="py-4 px-6">
                        <div class="flex items-center gap-2">
                            <button x-data @click="$dispatch('open-edit-modal', @js($l))" class="px-3 py-1.5 text-xs font-semibold bg-primary-50 text-primary rounded-lg hover:bg-primary hover:text-white transition-all duration-300">Edit</button>
                            <form method="POST" action="/admin/site-language/{{ $l->id }}/delete" x-data @submit.prevent="if(confirm('Delete this language?')) $el.submit()">
                                @csrf
                ...
                @empty
                <tr><td colspan="6" class="py-8 text-center text-heading/50 text-sm">No languages found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
<div x-data="{ open: false, item: {} }"
    ...
    <div class="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        <div class="p-6 border-b border-gray-100 flex items-center justify-between">
            <h3 class="font-bold text-heading">Edit Language</h3>
            <button @click="open = false" class="text-heading/40 hover:text-heading"><i class="ri-close-line text-xl"></i></button>
        </div>
        <form method="POST" :action="'/admin/site-language/' + item.id + '/update'" class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
            @csrf
