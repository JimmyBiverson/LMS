...
class CartController extends Controller
{
    ...
    public function placeOrder(Request $request): RedirectResponse
    {
        ...
        $enrolled = [];
        $hasPaidItems = false;
        $isOffline = $request->payment_method === 'offline';
                    ...
                    if ($isOffline) {
                        ...
                        if (!$exists) {
                            Enrollment::create([
                                'user_id' => $user->id,
                                'course_id' => $course->id,
                                'amount_paid' => $course->sale_price ?? $course->price,
        ...
        if ($hasPaidItems && !$isOffline) {
            return redirect('/checkout')->with('info', 'Paid items require payment. Please complete checkout.');
        }
        ...
        $message = $isOffline
            ? 'Your enrollment request is pending. The instructor will confirm your payment.'
            : 'Successfully enrolled in ' . count($enrolled) . ' course(s)!';

        if ($isOffline) {
            session()->flash('payment_method', 'offline');
        }

        return redirect('/dashboard/my-enrolled-course')->with('success', $message);
    }
}
