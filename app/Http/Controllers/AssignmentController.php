...
class AssignmentController extends Controller
{
    ...
    public function submitForm(Assignment $assignment): View
    {
        ...
        if ($assignment->due_date && $assignment->isDeadlinePassed() && !$assignment->late_submission_allowed) {
            return view('assignments.submit', compact('assignment'))->with('error', 'This assignment deadline has passed and late submissions are not allowed.');
        }
    ...
    public function submit(Request $request, Assignment $assignment): RedirectResponse
    {
        ...
        if ($assignment->due_date && $assignment->isDeadlinePassed() && !$assignment->late_submission_allowed) {
            abort(403, 'This assignment deadline has passed and late submissions are not allowed.');
        }
