...
class SettingsController extends Controller
{
    ...
    public function toggleSuperInstructor(User $user, Request $request): RedirectResponse
    {
        if ($user->role !== User::ROLE_INSTRUCTOR) {
            return back()->with('error', 'Only instructors can be marked as super instructors.');
        }

        $validated = $request->validate([
            'super_reason' => 'required|string|min:5|max:1000',
        ]);

        $becomingSuper = ! (bool) $user->is_super_instructor;

        $user->update([
            'is_super_instructor' => $becomingSuper,
            'super_reason' => $becomingSuper ? $validated['super_reason'] : null,
            'super_reason_date' => $becomingSuper ? now() : null,
        ]);

        return back()->with('success', $user->is_super_instructor
            ? "{$user->name} is now a super instructor."
            : "{$user->name} is no longer a super instructor.");
    }
}
