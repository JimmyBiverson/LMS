...
<section class="py-12 lg:py-16">
    <div class="max-w-7xl mx-auto px-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <a href="{{ route('users.profile', $instructor->id) }}" class="group bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 p-6 text-center block">
                <div class="w-24 h-24 rounded-full overflow-hidden mx-auto mb-4">
                    @if($instructor->profile_image)
                        <img src="{{ $instructor->profile_image_url }}" alt="{{ $instructor->full_name }}" class="w-full h-full object-cover">
                    @else
