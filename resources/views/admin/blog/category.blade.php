    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                @foreach(['Education','Technology','Business','Lifestyle'] as $i=>$c)
                <tr class="hover:bg-gray-50"><td class="py-4 px-6 text-heading/70">{{ $i+1 }}</td><td class="py-4 px-6 font-semibold text-heading">{{ $c }}</td><td class="py-4 px-6 text-heading/70">{{ strtolower($c) }}</td><td class="py-4 px-6 text-heading/70">{{ 3+$i*2 }}</td><td class="py-4 px-6"><span class="px-3 py-1 rounded-full text-xs font-bold bg-green-100 text-green-700">Active</span></td></tr>
                @endforeach
            </tbody>
        </table>
    </div>
