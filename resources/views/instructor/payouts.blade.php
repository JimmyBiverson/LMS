...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
    <div class="bg-white rounded-xl shadow-sm p-6"><p class="text-sm text-heading/60 mb-1">Total Earnings</p><p class="text-2xl font-bold text-heading">{{ \App\Helpers\CurrencyHelper::format($totalEarnings) }}</p></div>
    <div class="bg-white rounded-xl shadow-sm p-6"><p class="text-sm text-heading/60 mb-1">Total Paid</p><p class="text-2xl font-bold text-green-600">{{ \App\Helpers\CurrencyHelper::format($totalPaid) }}</p></div>
    <div class="bg-white rounded-xl shadow-sm p-6"><p class="text-sm text-heading/60 mb-1">Pending Balance</p><p class="text-2xl font-bold text-heading">{{ \App\Helpers\CurrencyHelper::format($pendingBalance) }}</p></div>
</div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6">
        ...
        <div class="space-y-3">
            ...
            <div class="flex items-center justify-between p-4 border border-gray-100 rounded-lg">
                <div><p class="font-semibold text-heading text-sm">{{ \App\Helpers\CurrencyHelper::format($payout->amount) }}</p><p class="text-xs text-heading/50">{{ $payout->method }} &middot; {{ $payout->created_at->format('M d, Y') }}</p></div>
